...
export function RecipeForm({
  ...
  const pickIngredientPhoto = async (id: string) => {
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: "images",
      quality: 0.85,
      allowsEditing: true,
    ...
    if (!result.canceled && result.assets[0]) {
      updateIngredient(id, { imageUrl: result.assets[0].uri });
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={{ gap: 10 }}>
          {ingredients.map((ing) => (
            <View key={ing.id} style={styles.ingredientEditor}>
              <View style={styles.ingredientRow}>
                <View>
                  <Pressable
                    style={styles.ingredientPhoto}
                    onPress={() => pickIngredientPhoto(ing.id)}
                    accessibilityLabel={
                      ing.imageUrl
                        ? "Replace ingredient picture"
                  ...
                  >
                    {ing.imageUrl ? (
                      <Image
                        ...
                        style={styles.ingredientPhotoImage}
                        contentFit="cover"
                      />
                    ) : (
                  ...
                  {ing.imageUrl && (
                    <Pressable
                      style={styles.ingredientPhotoRemove}
                      onPress={() =>
                        updateIngredient(ing.id, { imageUrl: null })
                      }
                      hitSlop={6}
                    >
                      <X size={9} color={colors.text} strokeWidth={3} />
                    </Pressable>
                  )}
                </View>
                <TextInput
              ...
              <View style={styles.chipWrap}>
                {UNITS.map((u) => (
                  <Chip
                    ...
                    label={u}
                    active={
                      ingredients.find((i) => i.id === unitPickerFor)?.unit ===
                      u
                    }
                    onPress={() => {
                      if (unitPickerFor) {
...
const createStyles = (colors: Palette) =>
  StyleSheet.create({
    ...
    ingredientDetails: { flexDirection: "row", gap: 8, paddingLeft: 52 },
    ingredientPhoto: {
      width: 44,
      height: 44,
      borderRadius: 22,
    ...
    ingredientPhotoImage: { width: "100%", height: "100%" },
    ingredientPhotoRemove: {
      position: "absolute",
      right: -4,
      top: -4,
      width: 17,
